...
-->
# 配色与字体方案库（Color Scheme - PPT）

> 来源：MiniMax-AI/skills → plugins/pptx-plugin/skills/color-font-skill。
> 用法：按场景选 1 套配色，映射为 PPTD theme 变量（映射表见 [themes.md](themes.md)）。**只允许使用本文件中的颜色，不发明新色。**
...
| 19 | 政务庄重（中国红） | `#A6192E` `#C8102E` `#8C6D1F` `#F7F5F0` `#FFFFFF` | 庄重、权威、严谨 | 政务汇报、国企总结、党建宣传、年度工作报告 | 中国红主调，白底黑字，金色点缀 |
| 20 | 学术深蓝（三色克制） | `#1F4E79` `#2E75B6` `#B08D2E` `#F5F8FC` `#FFFFFF` | 专业、克制、证据导向 | 学术报告、论文答辩、研究汇报、数据密集型 deck | 深藏青主调 + 中蓝强调，白底，三色克制（academic-pptx 吸收） |

---

## 二、Agent Design System - 完整色板（Platinum White-Gold）

基于 tokens.css/ts 的铂金白-金主题，提供完整色阶供精细设计使用。
...
> PPTD 编译器支持 `#RRGGBBAA` 8 位色值（含透明度），如 `#0a0a0a80` 表示 50% 黑遮罩。
...
- 中英文混排：中文使用微软雅黑，英文使用所选字体
- 优先使用系统自带字体，确保跨平台兼容
- 标题和正文可使用不同字体配对（如 Georgia + Calibri）
- **正文不加粗**：正文、图注、脚注用常规字重；加粗只给标题和表头
